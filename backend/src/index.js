...
async function runMigrations() {
  ...
  try {
    ...
    if (tenantId) {
      const pipeRes = await client.query(`INSERT INTO pipelines (tenant_id, name, is_default) VALUES ($1, 'Sales Pipeline', true) ON CONFLICT DO NOTHING RETURNING id`, [tenantId]);
      ...
      if (pipelineId) {
        ...
        for (const [name,pos,prob,color,won,lost] of stages) {
          await client.query(`INSERT INTO pipeline_stages (pipeline_id, name, position, probability_default, color, is_won, is_lost) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT DO NOTHING`, [pipelineId,name,pos,prob,color,won,lost]);
        }
      }
